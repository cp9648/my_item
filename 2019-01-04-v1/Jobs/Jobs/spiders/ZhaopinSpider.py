...
class ZhaopinspiderSpider(scrapy.Spider):
    ...
    def parse_city(self, response):
        ...
        for item in resp_dict['data']['results']:
            ...
            # 返回每一条数据
            # 职位名称
            _item['name'] = item['jobName']
            # 详细链接
            _item['detailed_url'] = item['positionURL']
            # 公司名称
            _item['company_name'] = item['company']['name']
            # 公司链接 
            _item['company_url'] = item['company']['url']
            # 工作地址
            _item['Work_address'] = item['city']['display']
            # 薪资
            _item['salary'] = item['salary']
            # 发布时间
            _item['time'] = item['createDate']
            # import ipdb; ipdb.set_trace()
            yield _item
